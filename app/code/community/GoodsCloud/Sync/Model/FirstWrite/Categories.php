...
class GoodsCloud_Sync_Model_FirstWrite_Categories
    ...
    public function createCategories(array $storeViews)
    {
        foreach ($storeViews as $view) {
            ...
            if (!$view->getGcChannelId()) {
                Mage::throwException(sprintf('Store %s has no gc channel id set!',
                        $view->getName()));
            }
            ...
            foreach ($this->categories as $category) {
                ...
                if (!isset($categoryIds[$view->getGcChannelId()])) {
                    ...
                    if ($category->getId() != $view->getRootCategoryId()) {
                        $gcParentId
                            = $this->categoryIdCache[$category->getParentId()];
                    }

                    $categoryData = $this->createCategory($category, $view,
                        $gcParentId);
                    ...
                    $categoryIds[$view->getGcChannelId()]
                        = $categoryData->getId();
                    $category->setGcCategoryIds(json_encode($categoryIds));
                    $category->save();
                }
                $this->categoryIdCache[$category->getId()]
                    = $categoryIds[$view->getGcChannelId()];
            }
        }
    }
